...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary()
                .switchOnFirst((signal, flux) -> Objects.requireNonNull(signal.get()).metaData.equals("spam") ? mailBoxSecondary() : mailBoxPrimary());
    ...
    public void car_factory() {
        //todo: feel free to change code as you need
        Flux<Car> producedCars = Flux.zip(carChassisProducer(), carEngineProducer()).map(tuple -> new Car(tuple.getT1(), tuple.getT2()));
